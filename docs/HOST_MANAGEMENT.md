...
`connpass` exposes whether the six-hour public-event refresh is enabled, its last/next fetch time, failure count and cached entry count without exposing the API key or event text.

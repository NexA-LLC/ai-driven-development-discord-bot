...
- Atomは固定HTTPS URLのみ、リダイレクト拒否、10秒、256KiB、最大100件を解析して20件採用。DTD/ENTITYを拒否するnamespace対応XMLパーサを使います。内容は非信頼の参照データで、命令として使いません。公開イベント紹介には私的会話の原文・人物情報を渡しません。

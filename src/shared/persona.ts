...
export const HOW_TO_TALK = [
  ...
  "・`@スー` またはスーの投稿への返信 … 対応チャンネルで会話を続けます",
  ...
  "私宛ての会話から短い根拠付きの記憶を作り、同じチャンネルの会話で最大30日参照します。削除された根拠は使いません。",
...
- 渡された根拠付きの経験記憶があれば、関連する会話で思い出して次の言葉に生かす。引用された事実と自分の解釈を分ける。渡されていない過去の会話・来店・体験を覚えているふりはしない。
...
- 実際の根拠がない会話やイベント参加・体験を作ること。公開イベントの掲載/更新日時を開催日時として扱うこと。

...
connpass API v2 からAI駆動開発グループ（`subdomain=aid`）のイベント一覧を6時間ごとに取得・キャッシュできます。タイトル・説明・開催/公開/更新日時・募集状態・参加集計・会場・画像などを保持します。connpassで発行されたAPI keyを `CONNPASS_API_KEY` に設定し、`CONNPASS_ENABLED=true` で有効化します。取得済みイベントはスーのメンション返答へ自動注入せず、「次のイベントは？」へ定型回答するイベント案内Botにはしません。一方、開催3日前・前日・当日の開始前には、通常の独り言枠でイベントをきっかけにした短い話を `EVENT_POST_CHANNEL_ID`（未設定なら `#雑談-general`）へ投稿できます。イベントと節目ごとに1回、イベント由来の投稿は全体で1日1件までです。`CONNPASS_POLL_SECONDS` / `CONNPASS_CACHE_HOURS` で取得を調整できます。公開情報はスー自身の体験記憶には登録しません。

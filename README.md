...
connpass API v2 からAI駆動開発グループ（`subdomain=aid`）のイベント一覧を6時間ごとに取得・キャッシュできます。タイトル・説明・開催/公開/更新日時・募集状態・参加集計・会場・画像などを保持します。connpassで発行されたAPI keyを `CONNPASS_API_KEY` に設定し、`CONNPASS_ENABLED=true` で有効化します。取得済みイベントはスーのメンション返答へ自動注入せず、「次のイベントは？」へ定型回答するイベント案内Botにはしません。一方、開催3日前・前日・当日の開始前には、通常の独り言枠でイベントをきっかけにした短い話を `EVENT_POST_CHANNEL_ID`（未設定なら `#雑談-general`）へ投稿します。イベント名・グループ・会場・住所から大阪、広島、福岡など既存支部と一致した場合は、同じ本文を該当支部チャンネルにも配信します。イベントと節目ごとに1回、イベント由来の主投稿は全体で1日1件までです。`CONNPASS_POLL_SECONDS` / `CONNPASS_CACHE_HOURS` で取得を調整できます。公開情報はスー自身の体験記憶には登録しません。

新規参加者へのwelcomeは、参加イベントを `welcome-queue.json` へ先に保存します。LLM生成を2回まで試し、それでも失敗した場合は短い固定挨拶へフォールバックします。生成済み本文と決定的nonceを再利用してDiscord送信を再試行し、message IDを受け取って初めて完了扱いにします。起動・再接続時は `WELCOME_BACKFILL_HOURS`（既定24時間）内の参加者を確認し、既存の成功receiptがない人だけ補完します。
